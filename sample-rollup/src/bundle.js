...
import cjs from "./finalisers/cjs.js";
import ExternalModule from "./external-module.js";
import replaceIdentifiers from "./utils/replaceIdentifiers.js";
...
export default class Bundle {
  ...
  async build() {
    const curEntryModule = await this.fetchModule(this.entryPath);
    this.entryModule = curEntryModule;
    const curStatements = await curEntryModule.expandAllStatements(true);
    this.statements = curStatements;
    this.deconflict();
  }
  ...
  async fetchModule(importee, importer) {
    return new Promise((resolve, reject) => {
      ...
      const route = getFileResolvePath(importee, importer);
      if (route) {
        try {
          const curCode = fs.readFileSync(route, 'utf-8');
          const curModule = new Module({
            code: curCode,
            path: route,
            bundle: this,
          });

          this.modules[route] = curModule;
          resolve(curModule);
        } catch (err) {
          reject(err);
        }
...
// 获取文件的绝对路径
function getFileResolvePath(importee, importer) {
  let curPath;
  ...
  if (!importer) {
    ...
    if (path.isAbsolute(importee)) {
      ...
      // 处理相对路径
      curPath = path.resolve(path.dirname(importer), importee);
    }
  }
